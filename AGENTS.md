...
Stages 1–6 are implemented. Gates: `tests/test_smoke.py` (async parameter server), `tests/test_staleness.py` (fixed bounded staleness), `tests/test_compression.py` (Top-K / quantization with exact wire-byte accounting), `tests/test_error_feedback.py` (EF conservation invariant), `tests/test_controller.py` (adaptive controller; `controller.enabled: false` reproduces the fixed policies), `tests/test_sync.py` (synchronous SGD rounds), and `tests/test_models.py` (ResNet-18/CIFAR). All six proposal baselines exist: five as pure configs plus `workload.sync: true` for synchronous SGD; the three ablation configs use `controller.adapt_compression` / `adapt_staleness` per-knob switches. `configs/matrix_cifar_full.yaml` (11 configs × 3 seeds) is the full evaluation matrix, launched with `scripts/run_matrix.py`; `scripts/plot_results.py` produces the summary table and comparison figures. GPU runs via `run.device: auto` (CUDA wheels required — see requirements.txt).
